...
public enum ExceptionEvent
{
    ...
    ENCRYPTION_ERROR = 7 // Decryption check failed
}
...
public class SvrVideoPlayer : MonoBehaviour
{
    ...
    /// <summary>
    /// Note: "SetEncryFile" and "SetEncryChannel" useful when you need to decrypt an encrypted video
    /// </summary>
    [DllImport(dllName)]
    private static extern int SetEncryFile(IntPtr videoPlayerPtr, string encryptionFile);
    [DllImport(dllName)]
    private static extern void SetEncryChannel(IntPtr videoPlayerPtr, string channel);
#endif
    #endregion
    ...
    public delegate void VideoPlayerStatus(bool status);
    ...
    public VideoPlayerStatus OnVideoPlayerStatusChange;
    ...
    public void PreparedPlayVideo(string url)
    {
        ...
        JVideoDescriptionInfo videoInfo = new JVideoDescriptionInfo(-1, name, url, url, 0, 0, 0, 0, DateTime.Now, blive);

        string json = JsonUtility.ToJson(videoInfo);
        IsVideoReady = false;

        // Useful when you need to decrypt an encrypted video. 
        // If you don't need decryption, you don't need to call this method.
        // The incoming channel number needs to match the channel number set in the encryption tool.
        // "SSNWT001" just an example.
        SetEncryChannel("SSNWT001");
        // You can also use the "SetEncryFile" method
        //SetEncryFile("/storage/emulated/0/SSNWT001.enc");
    ...
    public void Play()
    {
        ...
        if (IsVideoReady)
        {
            if (OnVideoPlayerStatusChange != null)
                OnVideoPlayerStatusChange(true);
    ...
    public void Pause()
    {
        ...
        if (OnVideoPlayerStatusChange != null)
            OnVideoPlayerStatusChange(false);
    ...
    /// Useful when you need to decrypt an encrypted video. 
    /// If you don't need decryption, you don't need to call this method.
    /// Enter the full path to the encrypted profile. The encrypted profile has the suffix with ".enc".
    /// If you don't know the full path, you can use the "SetEncryChannel" method.
    /// </summary>
    /// <param name="encryptionFile"></param>
    public void SetEncryFile(string encryptionFile)
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        if (VideoPlayerPtr == IntPtr.Zero)
            return;

        SetEncryFile(VideoPlayerPtr, encryptionFile);
#endif
    }
    ...
    /// If you don't need decryption, you don't need to call this method.
    /// The incoming channel number needs to match the channel number set in the encryption tool.
    /// If you don't know the channel number, you can use the "SetEncryFile" method.
    /// </summary>
    /// <param name="channel"></param>
    public void SetEncryChannel(string channel)
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        if (VideoPlayerPtr == IntPtr.Zero)
            return;

        SetEncryChannel(VideoPlayerPtr, channel);
#endif
    }
